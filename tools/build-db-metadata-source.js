...
const tableDescriptions = {
  ...
  user_groups: ["ユーザーグループ", "管理者が扱うユーザー集合と権限付与単位を表す。状態はprojectionとして扱う。"],
  ...
  chat_message_events: ["チャットメッセージUIイベント", "UIへ返す軽量イベント列。domain event正本とは分け、通知・REST取得のための時系列read modelとして扱う。"],
  ...
  ws_tickets: ["WebSocket ticket", "AppSync Events購読前にHono APIが発行する短期・単回利用ticketを表す。状態はprojectionとして扱う。"],
  user_import_jobs: ["ユーザー取込ジョブ", "管理者によるユーザー一括取込の実行単位を表す。状態は user_import_job_events のprojectionとして扱う。"],
  user_import_rows: ["ユーザー取込行", "ユーザー一括取込の行単位結果を保持する。行状態は取込ジョブ処理から導出されるprojectionである。"],
  evaluation_datasets: ["評価データセット", "RAG評価またはLLM評価で使う問題集合を管理する。状態は管理操作から導出されるprojectionとして扱う。"],
  ...
  evaluation_run_items: ["評価実行項目", "評価実行に含まれるケース単位の回答、検索文脈、judge結果、metricsを保持する。"],
  llm_models: ["LLMモデルカタログ", "一般ユーザーや評価基盤が利用可能なモデル定義を共有する。表示状態はカタログprojectionとして扱う。"],
  bm25_search_documents: ["BM25F検索文書", "Sparse retrieval用の文書read model。is_deleted は検索除外projectionであり正本ではない。"],
  bm25_postings: ["BM25F posting", "termと文書fieldの出現頻度を保持する転置インデックス本体。"],
  bm25_term_stats: ["BM25F term統計", "termごとのdf/idfを保持する検索用read model。"],
  bm25_field_stats: ["BM25F field統計", "fieldごとの平均長を保持する検索用read model。"],
  event_delivery_logs: ["イベント配信ログ", "AppSync Eventsなどへの通知配信試行と結果を保持するoperations projection。"],
  audit_events: ["監査イベント", "管理操作、チャット共有、Tools実行、成果物アクセスなど監査対象のappend-onlyログ。"],
  agent_tools: ["Agent Tool定義", "AgentCore Gatewayから利用するtoolのschema、scope、timeout、副作用分類を管理する。"],
...
function columnMetadata(tableName, column, projection) {
  ...
  const derivedFrom = isProjectionColumn && projection ? projection.eventTable : undefined;
  return {
    ...column,
    ...
    updateOwner: updateOwnerFor(tableName, column.name, isProjectionColumn, projection)
  };
}
...
function columnDescription(tableName, name, isProjectionColumn, projection) {
  if (isProjectionColumn) {
    const source = projection?.eventTable ? `${projection.eventTable} から導出される` : "対応するdomain eventから導出される";
