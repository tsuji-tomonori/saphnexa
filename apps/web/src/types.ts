...
export type ChatMessage = { message_id: string; parent_message_id?: string | null; sender_user_id?: string | null; sender_type: "general_user" | "admin" | "assistant"; content_text: string; run_id?: string | null; status: string; created_at: string; completed_at?: string | null; feedback?: MessageFeedback | null; citations?: Citation[] };
...
export type IngestionJob = { job_id: string; document_id: string; version_id: string; status: string; progress_percent: number; raw_s3_uri: string; parsed_s3_prefix: string; error_code?: string | null; retryable: boolean };
...
export type LlmModel = { model_id: string; display_name: string; provider: string; model_type: string; capability_json: Record<string, unknown>; status: string; visible_to_user: boolean; allowed_role: string; default_for_task: string; catalog_version: string };
...
export type EvaluationRunItem = { case_id: string; status: string; answer_text?: string | null; retrieved_context_json?: Record<string, unknown> | null; judge_result_json?: Record<string, unknown> | null; metrics_json?: Record<string, unknown> | null };
