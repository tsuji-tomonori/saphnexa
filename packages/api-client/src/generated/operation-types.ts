/*
 * Generated by tools/build-api-client-operation-types.js.
 * Do not edit by hand.
 */

export type ApiClientJsonObject = Record<string, unknown>;
export type ApiClientEmptyObject = Record<string, never>;

export interface ApiClientErrorResponse {
  trace_id: string;
  error_code: string;
  message: string;
  details: ApiClientJsonObject;
}

export type ApiClientGeneratedOperationId = "loginStart" | "authCallback" | "logout" | "getMe" | "listChatSessions" | "createChatSession" | "getChatSession" | "updateChatSession" | "deleteChatSession" | "listChatParticipants" | "addChatParticipant" | "updateChatParticipant" | "removeChatParticipant" | "listMessages" | "submitQuestion" | "listMessageEvents" | "cancelAnswerGeneration" | "createFeedback" | "listFavorites" | "addFavorite" | "deleteFavorite" | "issueWsTicket" | "listLlmModels" | "adminListUsers" | "startUserImport" | "getUserImport" | "adminListDocuments" | "createDocument" | "getDocument" | "createDocumentVersion" | "activateDocumentVersion" | "updateDocumentAcl" | "suspendDocument" | "getIngestionJob" | "retryIngestionJob" | "listEvaluationDatasets" | "startEvaluationRun" | "getEvaluationRun" | "listPublishedArtifacts" | "issueArtifactAccessCookie";

export interface ApiClientOperationTypes {
  loginStart: {
    operationId: "loginStart";
    method: "GET";
    viewerPath: "/auth/login";
    internalPath: "/v1/auth/login";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 302;
    successResponse: void;
    errorResponse: ApiClientErrorResponse;
  };
  authCallback: {
    operationId: "authCallback";
    method: "GET";
    viewerPath: "/auth/callback";
    internalPath: "/v1/auth/callback";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 302 | 400;
    successResponse: void;
    errorResponse: ApiClientErrorResponse;
  };
  logout: {
    operationId: "logout";
    method: "POST";
    viewerPath: "/auth/logout";
    internalPath: "/v1/auth/logout";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 204;
    successResponse: void;
    errorResponse: ApiClientErrorResponse;
  };
  getMe: {
    operationId: "getMe";
    method: "GET";
    viewerPath: "/api/me";
    internalPath: "/v1/me";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { user: { tenant_id: string; user_id: string; email: string; display_name: string; role: "general_user" | "admin" | "system"; department?: string; employment_type?: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; created_at?: string; updated_at?: string }; csrf_token: string };
    errorResponse: ApiClientErrorResponse;
  };
  listChatSessions: {
    operationId: "listChatSessions";
    method: "GET";
    viewerPath: "/api/chat-sessions";
    internalPath: "/v1/chat-sessions";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { chats: { tenant_id: string; chat_id: string; title: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; last_message_at?: string | null; created_by_user_id: string; created_at: string; updated_at: string; deleted_at?: string | null }[] };
    errorResponse: ApiClientErrorResponse;
  };
  createChatSession: {
    operationId: "createChatSession";
    method: "POST";
    viewerPath: "/api/chat-sessions";
    internalPath: "/v1/chat-sessions";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 201;
    successResponse: { chat: { tenant_id: string; chat_id: string; title: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; last_message_at?: string | null; created_by_user_id: string; created_at: string; updated_at: string; deleted_at?: string | null } };
    errorResponse: ApiClientErrorResponse;
  };
  getChatSession: {
    operationId: "getChatSession";
    method: "GET";
    viewerPath: "/api/chat-sessions/{chat_id}";
    internalPath: "/v1/chat-sessions/{chat_id}";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: { chat_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { chat: { tenant_id: string; chat_id: string; title: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; last_message_at?: string | null; created_by_user_id?: string; created_at?: string; updated_at?: string; deleted_at?: string | null; participants: { tenant_id: string; chat_id: string; user_id: string; participant_role: "owner" | "viewer"; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; added_by_user_id: string; added_at: string; removed_at?: string | null }[]; messages: { tenant_id: string; chat_id: string; message_id: string; parent_message_id?: string | null; sender_user_id?: string | null; sender_type: "general_user" | "admin" | "assistant"; content_text: string; run_id?: string | null; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; created_at: string; completed_at?: string | null; feedback?: { tenant_id: string; feedback_id: string; user_id: string; chat_id?: string | null; message_id?: string | null; rating?: string; comment?: string | null; problem_type?: string | null; created_at: string } & ApiClientJsonObject | null; citations?: { tenant_id: string; chat_id: string; message_id: string; citation_id: string; document_id: string; version_id: string; chunk_id: string; display: ApiClientJsonObject }[] }[] } };
    errorResponse: ApiClientErrorResponse;
  };
  updateChatSession: {
    operationId: "updateChatSession";
    method: "PATCH";
    viewerPath: "/api/chat-sessions/{chat_id}";
    internalPath: "/v1/chat-sessions/{chat_id}";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { chat_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 200;
    successResponse: { chat: { tenant_id: string; chat_id: string; title: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; last_message_at?: string | null; created_by_user_id: string; created_at: string; updated_at: string; deleted_at?: string | null } };
    errorResponse: ApiClientErrorResponse;
  };
  deleteChatSession: {
    operationId: "deleteChatSession";
    method: "DELETE";
    viewerPath: "/api/chat-sessions/{chat_id}";
    internalPath: "/v1/chat-sessions/{chat_id}";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { chat_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 204;
    successResponse: void;
    errorResponse: ApiClientErrorResponse;
  };
  listChatParticipants: {
    operationId: "listChatParticipants";
    method: "GET";
    viewerPath: "/api/chat-sessions/{chat_id}/participants";
    internalPath: "/v1/chat-sessions/{chat_id}/participants";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: { chat_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { participants: { tenant_id: string; chat_id: string; user_id: string; participant_role: "owner" | "viewer"; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; added_by_user_id: string; added_at: string; removed_at?: string | null }[] };
    errorResponse: ApiClientErrorResponse;
  };
  addChatParticipant: {
    operationId: "addChatParticipant";
    method: "POST";
    viewerPath: "/api/chat-sessions/{chat_id}/participants";
    internalPath: "/v1/chat-sessions/{chat_id}/participants";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { chat_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 201;
    successResponse: { participant: { tenant_id: string; chat_id: string; user_id: string; participant_role: "owner" | "viewer"; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; added_by_user_id: string; added_at: string; removed_at?: string | null } };
    errorResponse: ApiClientErrorResponse;
  };
  updateChatParticipant: {
    operationId: "updateChatParticipant";
    method: "PATCH";
    viewerPath: "/api/chat-sessions/{chat_id}/participants/{user_id}";
    internalPath: "/v1/chat-sessions/{chat_id}/participants/{user_id}";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { chat_id: string; user_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 200;
    successResponse: { participant: { tenant_id: string; chat_id: string; user_id: string; participant_role: "owner" | "viewer"; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; added_by_user_id: string; added_at: string; removed_at?: string | null } };
    errorResponse: ApiClientErrorResponse;
  };
  removeChatParticipant: {
    operationId: "removeChatParticipant";
    method: "DELETE";
    viewerPath: "/api/chat-sessions/{chat_id}/participants/{user_id}";
    internalPath: "/v1/chat-sessions/{chat_id}/participants/{user_id}";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { chat_id: string; user_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 204;
    successResponse: void;
    errorResponse: ApiClientErrorResponse;
  };
  listMessages: {
    operationId: "listMessages";
    method: "GET";
    viewerPath: "/api/chat-sessions/{chat_id}/messages";
    internalPath: "/v1/chat-sessions/{chat_id}/messages";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: { chat_id: string };
    query: { after_message_id?: string; limit?: number };
    requestBody: never;
    successStatus: 200;
    successResponse: { messages: { tenant_id: string; chat_id: string; message_id: string; parent_message_id?: string | null; sender_user_id?: string | null; sender_type: "general_user" | "admin" | "assistant"; content_text: string; run_id?: string | null; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; created_at: string; completed_at?: string | null; feedback?: { tenant_id: string; feedback_id: string; user_id: string; chat_id?: string | null; message_id?: string | null; rating?: string; comment?: string | null; problem_type?: string | null; created_at: string } & ApiClientJsonObject | null; citations?: { tenant_id: string; chat_id: string; message_id: string; citation_id: string; document_id: string; version_id: string; chunk_id: string; display: ApiClientJsonObject }[] }[]; next_cursor: string | null };
    errorResponse: ApiClientErrorResponse;
  };
  submitQuestion: {
    operationId: "submitQuestion";
    method: "POST";
    viewerPath: "/api/chat-sessions/{chat_id}/messages";
    internalPath: "/v1/chat-sessions/{chat_id}/messages";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { chat_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 202;
    successResponse: { message_id: string; run_id: string; status: string };
    errorResponse: ApiClientErrorResponse;
  };
  listMessageEvents: {
    operationId: "listMessageEvents";
    method: "GET";
    viewerPath: "/api/chat-sessions/{chat_id}/messages/{message_id}/events";
    internalPath: "/v1/chat-sessions/{chat_id}/messages/{message_id}/events";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: { chat_id: string; message_id: string };
    query: { after_seq?: number };
    requestBody: never;
    successStatus: 200;
    successResponse: { events: { tenant_id: string; chat_id: string; message_id: string; event_seq: number; event_id: string; event_name: string; event_type: "progress" | "partial" | "final" | "error"; payload_json: ApiClientJsonObject; created_at: string }[] };
    errorResponse: ApiClientErrorResponse;
  };
  cancelAnswerGeneration: {
    operationId: "cancelAnswerGeneration";
    method: "POST";
    viewerPath: "/api/chat-sessions/{chat_id}/messages/{message_id}/cancel";
    internalPath: "/v1/chat-sessions/{chat_id}/messages/{message_id}/cancel";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { chat_id: string; message_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 202;
    successResponse: { message_id: string; run_id: string; status: string };
    errorResponse: ApiClientErrorResponse;
  };
  createFeedback: {
    operationId: "createFeedback";
    method: "POST";
    viewerPath: "/api/chat-sessions/{chat_id}/messages/{message_id}/feedback";
    internalPath: "/v1/chat-sessions/{chat_id}/messages/{message_id}/feedback";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { chat_id: string; message_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 201;
    successResponse: { feedback: { tenant_id: string; feedback_id: string; user_id: string; chat_id?: string | null; message_id?: string | null; rating?: string; comment?: string | null; problem_type?: string | null; created_at: string } & ApiClientJsonObject };
    errorResponse: ApiClientErrorResponse;
  };
  listFavorites: {
    operationId: "listFavorites";
    method: "GET";
    viewerPath: "/api/favorites";
    internalPath: "/v1/favorites";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { favorites: { tenant_id: string; favorite_id: string; user_id: string; chat_id?: string | null; message_id?: string | null; created_at: string }[] };
    errorResponse: ApiClientErrorResponse;
  };
  addFavorite: {
    operationId: "addFavorite";
    method: "POST";
    viewerPath: "/api/favorites";
    internalPath: "/v1/favorites";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 201;
    successResponse: { favorite: { tenant_id: string; favorite_id: string; user_id: string; chat_id?: string | null; message_id?: string | null; created_at: string } };
    errorResponse: ApiClientErrorResponse;
  };
  deleteFavorite: {
    operationId: "deleteFavorite";
    method: "DELETE";
    viewerPath: "/api/favorites/{favorite_id}";
    internalPath: "/v1/favorites/{favorite_id}";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: { favorite_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 204;
    successResponse: void;
    errorResponse: ApiClientErrorResponse;
  };
  issueWsTicket: {
    operationId: "issueWsTicket";
    method: "POST";
    viewerPath: "/api/ws-ticket";
    internalPath: "/v1/ws-ticket";
    roles: "general_user" | "admin";
    csrfRequired: true;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 201;
    successResponse: { ticket: string; expires_in_seconds: number; channels: string[] };
    errorResponse: ApiClientErrorResponse;
  };
  listLlmModels: {
    operationId: "listLlmModels";
    method: "GET";
    viewerPath: "/api/llm-models";
    internalPath: "/v1/llm-models";
    roles: "general_user" | "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { models: { tenant_id: "global"; model_id: string; display_name: string; provider: "bedrock"; model_type: "chat" | "judge" | "embedding"; capability_json: ApiClientJsonObject; status: "active" | "inactive"; visible_to_user: boolean; allowed_role: "general_user" | "admin" | "system"; default_for_task: string; catalog_version: string }[] };
    errorResponse: ApiClientErrorResponse;
  };
  adminListUsers: {
    operationId: "adminListUsers";
    method: "GET";
    viewerPath: "/api/admin/users";
    internalPath: "/v1/admin/users";
    roles: "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { users: { tenant_id: string; user_id: string; email: string; display_name: string; role: "general_user" | "admin" | "system"; department?: string; employment_type?: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; created_at?: string; updated_at?: string }[] };
    errorResponse: ApiClientErrorResponse;
  };
  startUserImport: {
    operationId: "startUserImport";
    method: "POST";
    viewerPath: "/api/admin/user-imports";
    internalPath: "/v1/admin/user-imports";
    roles: "admin";
    csrfRequired: true;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 202;
    successResponse: { import: { tenant_id: string; import_id: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; result_s3_prefix: string; result_report_json: { created: number; updated: number; deleted: number; failed: number; error_rows_s3_uri: string }; created_by_user_id: string } };
    errorResponse: ApiClientErrorResponse;
  };
  getUserImport: {
    operationId: "getUserImport";
    method: "GET";
    viewerPath: "/api/admin/user-imports/{import_id}";
    internalPath: "/v1/admin/user-imports/{import_id}";
    roles: "admin";
    csrfRequired: false;
    params: { import_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { import: { tenant_id: string; import_id: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; result_s3_prefix: string; result_report_json: { created: number; updated: number; deleted: number; failed: number; error_rows_s3_uri: string }; created_by_user_id: string }; rows: { tenant_id: string; import_id: string; row_number: number; action: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; target_user_id?: string | null; error_message?: string | null }[] };
    errorResponse: ApiClientErrorResponse;
  };
  adminListDocuments: {
    operationId: "adminListDocuments";
    method: "GET";
    viewerPath: "/api/admin/documents";
    internalPath: "/v1/admin/documents";
    roles: "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { documents: { tenant_id: string; document_id: string; title: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; created_by_user_id: string; created_at: string; updated_at: string }[] };
    errorResponse: ApiClientErrorResponse;
  };
  createDocument: {
    operationId: "createDocument";
    method: "POST";
    viewerPath: "/api/admin/documents";
    internalPath: "/v1/admin/documents";
    roles: "admin";
    csrfRequired: true;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 202;
    successResponse: { document_id: string; version_id: string; job_id: string; raw_s3_uri: string; idempotent?: boolean };
    errorResponse: ApiClientErrorResponse;
  };
  getDocument: {
    operationId: "getDocument";
    method: "GET";
    viewerPath: "/api/admin/documents/{document_id}";
    internalPath: "/v1/admin/documents/{document_id}";
    roles: "admin";
    csrfRequired: false;
    params: { document_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { document: { tenant_id: string; document_id: string; title: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; created_by_user_id: string; created_at: string; updated_at: string; versions: { tenant_id: string; document_id: string; version_id: string; version_label: string; status: "active" | "archived" | "deleted" | "failed" | "queued" | "succeeded" | "uploaded"; raw_s3_uri: string; metadata_json: ApiClientJsonObject; created_at: string }[]; ingestion_jobs: { tenant_id: string; job_id: string; document_id: string; version_id: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; progress_percent: number; raw_s3_uri: string; parsed_s3_prefix: string; error_code?: string | null; retryable: boolean }[]; acl_entries: { tenant_id: string; document_id: string; version_id: string; acl_scope_id: string; effect: "allow" | "deny" }[] } };
    errorResponse: ApiClientErrorResponse;
  };
  createDocumentVersion: {
    operationId: "createDocumentVersion";
    method: "POST";
    viewerPath: "/api/admin/documents/{document_id}/versions";
    internalPath: "/v1/admin/documents/{document_id}/versions";
    roles: "admin";
    csrfRequired: true;
    params: { document_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 202;
    successResponse: { document_id: string; version_id: string; job_id: string; raw_s3_uri: string; idempotent?: boolean };
    errorResponse: ApiClientErrorResponse;
  };
  activateDocumentVersion: {
    operationId: "activateDocumentVersion";
    method: "POST";
    viewerPath: "/api/admin/documents/{document_id}/versions/{version_id}/activate";
    internalPath: "/v1/admin/documents/{document_id}/versions/{version_id}/activate";
    roles: "admin";
    csrfRequired: true;
    params: { document_id: string; version_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 200;
    successResponse: { version: { tenant_id: string; document_id: string; version_id: string; version_label: string; status: "active" | "archived" | "deleted" | "failed" | "queued" | "succeeded" | "uploaded"; raw_s3_uri: string; metadata_json: ApiClientJsonObject; created_at: string } };
    errorResponse: ApiClientErrorResponse;
  };
  updateDocumentAcl: {
    operationId: "updateDocumentAcl";
    method: "POST";
    viewerPath: "/api/admin/documents/{document_id}/versions/{version_id}/acl";
    internalPath: "/v1/admin/documents/{document_id}/versions/{version_id}/acl";
    roles: "admin";
    csrfRequired: true;
    params: { document_id: string; version_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 200;
    successResponse: { document: { tenant_id: string; document_id: string; title: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; created_by_user_id: string; created_at: string; updated_at: string; versions: { tenant_id: string; document_id: string; version_id: string; version_label: string; status: "active" | "archived" | "deleted" | "failed" | "queued" | "succeeded" | "uploaded"; raw_s3_uri: string; metadata_json: ApiClientJsonObject; created_at: string }[]; ingestion_jobs: { tenant_id: string; job_id: string; document_id: string; version_id: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; progress_percent: number; raw_s3_uri: string; parsed_s3_prefix: string; error_code?: string | null; retryable: boolean }[]; acl_entries: { tenant_id: string; document_id: string; version_id: string; acl_scope_id: string; effect: "allow" | "deny" }[] } };
    errorResponse: ApiClientErrorResponse;
  };
  suspendDocument: {
    operationId: "suspendDocument";
    method: "POST";
    viewerPath: "/api/admin/documents/{document_id}/suspend";
    internalPath: "/v1/admin/documents/{document_id}/suspend";
    roles: "admin";
    csrfRequired: true;
    params: { document_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 200;
    successResponse: { document: { tenant_id: string; document_id: string; title: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; created_by_user_id: string; created_at: string; updated_at: string; versions: { tenant_id: string; document_id: string; version_id: string; version_label: string; status: "active" | "archived" | "deleted" | "failed" | "queued" | "succeeded" | "uploaded"; raw_s3_uri: string; metadata_json: ApiClientJsonObject; created_at: string }[]; ingestion_jobs: { tenant_id: string; job_id: string; document_id: string; version_id: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; progress_percent: number; raw_s3_uri: string; parsed_s3_prefix: string; error_code?: string | null; retryable: boolean }[]; acl_entries: { tenant_id: string; document_id: string; version_id: string; acl_scope_id: string; effect: "allow" | "deny" }[] } };
    errorResponse: ApiClientErrorResponse;
  };
  getIngestionJob: {
    operationId: "getIngestionJob";
    method: "GET";
    viewerPath: "/api/admin/ingestion-jobs/{job_id}";
    internalPath: "/v1/admin/ingestion-jobs/{job_id}";
    roles: "admin";
    csrfRequired: false;
    params: { job_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { job: { tenant_id: string; job_id: string; document_id: string; version_id: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; progress_percent: number; raw_s3_uri: string; parsed_s3_prefix: string; error_code?: string | null; retryable: boolean } };
    errorResponse: ApiClientErrorResponse;
  };
  retryIngestionJob: {
    operationId: "retryIngestionJob";
    method: "POST";
    viewerPath: "/api/admin/ingestion-jobs/{job_id}/retry";
    internalPath: "/v1/admin/ingestion-jobs/{job_id}/retry";
    roles: "admin";
    csrfRequired: true;
    params: { job_id: string };
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 202;
    successResponse: { job: { tenant_id: string; job_id: string; document_id: string; version_id: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; progress_percent: number; raw_s3_uri: string; parsed_s3_prefix: string; error_code?: string | null; retryable: boolean } };
    errorResponse: ApiClientErrorResponse;
  };
  listEvaluationDatasets: {
    operationId: "listEvaluationDatasets";
    method: "GET";
    viewerPath: "/api/admin/evaluation-datasets";
    internalPath: "/v1/admin/evaluation-datasets";
    roles: "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { datasets: { tenant_id: string; dataset_id: string; dataset_name: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; source_s3_uri: string; created_at: string }[] };
    errorResponse: ApiClientErrorResponse;
  };
  startEvaluationRun: {
    operationId: "startEvaluationRun";
    method: "POST";
    viewerPath: "/api/admin/evaluation-runs";
    internalPath: "/v1/admin/evaluation-runs";
    roles: "admin";
    csrfRequired: true;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 202;
    successResponse: { evaluation_run: { tenant_id: string; evaluation_run_id: string; dataset_id: string; model_id: string; prompt_version: string; retrieval_config_json: { top_k: number } & ApiClientJsonObject; artifact_s3_prefix: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; metrics_json: { retrieval?: { recall_at_10?: number } & ApiClientJsonObject; generation?: { groundedness?: number } & ApiClientJsonObject; end_to_end?: { refusal_accuracy?: number } & ApiClientJsonObject } & ApiClientJsonObject; created_by_user_id: string } };
    errorResponse: ApiClientErrorResponse;
  };
  getEvaluationRun: {
    operationId: "getEvaluationRun";
    method: "GET";
    viewerPath: "/api/admin/evaluation-runs/{evaluation_run_id}";
    internalPath: "/v1/admin/evaluation-runs/{evaluation_run_id}";
    roles: "admin";
    csrfRequired: false;
    params: { evaluation_run_id: string };
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { evaluation_run: { tenant_id: string; evaluation_run_id: string; dataset_id: string; model_id: string; prompt_version: string; retrieval_config_json: { top_k: number } & ApiClientJsonObject; artifact_s3_prefix: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; metrics_json: { retrieval?: { recall_at_10?: number } & ApiClientJsonObject; generation?: { groundedness?: number } & ApiClientJsonObject; end_to_end?: { refusal_accuracy?: number } & ApiClientJsonObject } & ApiClientJsonObject; created_by_user_id: string }; items: { tenant_id: string; evaluation_run_id: string; case_id: string; status: "active" | "archived" | "deleted" | "removed" | "queued" | "running" | "streaming" | "succeeded" | "failed" | "canceled"; answer_text?: string | null; retrieved_context_json?: ApiClientJsonObject | null; judge_result_json?: ApiClientJsonObject | null; metrics_json: ApiClientJsonObject | null }[] };
    errorResponse: ApiClientErrorResponse;
  };
  listPublishedArtifacts: {
    operationId: "listPublishedArtifacts";
    method: "GET";
    viewerPath: "/api/admin/artifacts";
    internalPath: "/v1/admin/artifacts";
    roles: "admin";
    csrfRequired: false;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: never;
    successStatus: 200;
    successResponse: { artifacts: { tenant_id: string; artifact_id: string; artifact_type: "design_doc_html" | "allure_report"; title: string; version_label?: string; source_ref: string; s3_bucket?: string; s3_prefix?: string; viewer_path: string; manifest_path?: string; status: string; checksum?: string; published_by?: string; published_at?: string; expires_at?: string | null; created_at?: string; updated_at?: string }[] };
    errorResponse: ApiClientErrorResponse;
  };
  issueArtifactAccessCookie: {
    operationId: "issueArtifactAccessCookie";
    method: "POST";
    viewerPath: "/api/admin/artifacts/access-cookie";
    internalPath: "/v1/admin/artifacts/access-cookie";
    roles: "admin";
    csrfRequired: true;
    params: ApiClientEmptyObject;
    query: ApiClientEmptyObject;
    requestBody: { csrf_token: string; title?: string; question?: string; user_id?: string; document_id?: string; version_id?: string; version_label?: string; file_name?: string; acl_scope_id?: string; dataset_id?: string; model_id?: string; import_id?: string; job_id?: string; now_ms?: number; rows?: ApiClientJsonObject[]; metadata?: ApiClientJsonObject; retrieval_policy?: { top_k?: number; allowed_acl_scope_ids?: string[] } } & ApiClientJsonObject;
    successStatus: 201;
    successResponse: { cookie_issued: boolean; expires_in_seconds: number };
    errorResponse: ApiClientErrorResponse;
  };
}

export type ApiClientOperationType<T extends ApiClientGeneratedOperationId> = ApiClientOperationTypes[T];
export type ApiClientOperationRequest<T extends ApiClientGeneratedOperationId> = ApiClientOperationTypes[T]["requestBody"];
export type ApiClientOperationResponse<T extends ApiClientGeneratedOperationId> = ApiClientOperationTypes[T]["successResponse"];
