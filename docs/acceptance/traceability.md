...
| AC-030 | local_verified | `packages/api-contract/src/routes.js` 40 routes and `tools/check-contracts.js`. |
...
| AC-153 | requires_aws | `docs/acceptance/defects/open_issues_snapshot.json` は local snapshot として Blocker/Critical open defect 0 件を検査するが、最終検収では `gh issue list --state open --json number,title,labels,state` を再取得し、`npm run acceptance:external-actions:check` / `npm run acceptance:final:check` で defect snapshot refresh action を pending 追跡する。 |
