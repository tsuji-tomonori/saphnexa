...
`chat_message_events` はUI通知とREST再取得のための時系列read modelであり、domain event正本とは分けて扱う。業務状態の正本は `chat_session_events`、`chat_participant_events`、`chat_run_events` などのappend-only event tableである。
...
| `user_groups` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
...
| `chat_messages` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `chat_messages` | `completed_at` | 対応するdomain event | 完了at。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
...
| `ws_tickets` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `ws_tickets` | `expires_at` | 対応するdomain event | 期限at。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `ws_tickets` | `used_at` | 対応するdomain event | usedat。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
...
| `user_import_rows` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `evaluation_datasets` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
...
| `evaluation_run_items` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `llm_models` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `llm_models` | `updated_at` | 対応するdomain event | 更新at。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `bm25_search_documents` | `is_deleted` | 対応するdomain event | is削除。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `event_delivery_logs` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `event_delivery_logs` | `updated_at` | 対応するdomain event | 更新at。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `agent_tools` | `status` | 対応するdomain event | 現在状態projection。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
| `agent_tools` | `updated_at` | 対応するdomain event | 更新at。対応するdomain eventから導出される読み取り最適化値であり、状態の正本ではなくprojectionである。 |
