...
resolved operator input が pass したら、外部実行前に operator execution runbook を生成・検査する。runbook は release、deploy_publish、preflight_capture、preflight_materialization、validation_capture、validation_materialization、final_gates、final_acceptance の順序、各 phase の `requires_confirmation`、`stop_on_failure`、証跡出力を固定する。validation_materialization phase には validation evidence build 後、`npm run test:e2e:aws`、`npm run perf:aws`、`npm run rag:quality:aws`、`npm run aws:dev-uat:validation:final` の順で suite gate commands を含める。未解決 operator input では `requires_resolved_operator_input` のまま残り、resolved mode では `<...>`、`sample`、`mock`、`localhost` を含む command を reject する。
...
`npm run aws:dev-uat:operator-runbook:check` は operator execution runbook を生成し、未解決 operator input では external execution ready と扱わず、resolved input がある場合だけ外部実行用の command order、confirmation gate、stop condition、evidence output、validation suite gate command order を検査する。この command は runbook を書き出すだけで、AWS command の実行や外部状態変更は行わない。
`npm run aws:dev-uat:operator-runbook:fixture:check` は operator execution runbook の resolved ready path、validation suite gate command order、placeholder 混入、確認なし外部 phase、phase order mismatch の negative path を検査する。fixture は最終検収 evidence として扱わない。
...
`npm run aws:dev-uat:final-readiness:check` は final readiness manifest を生成し、raw capture plan、execution bridge、preflight/validation raw input、final evidence、evidence bundle manifest、blockers、next commands を記録する。実 evidence と AWS credentials が揃わない場合は `blocked_by_external_execution` として残し、`--require-ready` では fail する。evidence bundle manifest は存在だけでなく、`schema_version`、`status`、`evidence_class`、current git commit、artifact count、artifact coverage、current readiness artifact path match、current artifact digest/size match、all bundle artifact metadata match、all bundle artifact scope match を検査する。
final readiness は raw input も必須条件として扱う。`dist/acceptance/raw/aws_dev_uat_preflight.raw.json` または `dist/acceptance/raw/aws_dev_uat_validation.raw.json` がない場合は `missing_preflight_raw_input` / `missing_validation_raw_input` を blocker とし、`npm run aws:dev-uat:raw-capture-plan:check`、`npm run aws:dev-uat:raw-input-scaffold:check`、materialize command、raw output check、raw input check、final evidence build、final gate command を next command に出す。これにより missing raw input から capture plan / scaffold / materialize / raw output-input check / final evidence build / final gate まで辿れる。
final readiness は resolved operator input も必須条件として扱う。`dist/acceptance/aws_dev_uat_operator_input.json` がない場合は `missing_operator_input`、placeholder や不正 URL / 不正 AWS account id が残る場合は `invalid_operator_input` を blocker とする。missing operator input では `npm run aws:dev-uat:operator-input:build`、`npm run aws:dev-uat:operator-input:check`、`npm run aws:dev-uat:operator-input:check -- --input dist/acceptance/aws_dev_uat_operator_input.json --require-resolved` を next command に出し、scaffold 生成から resolved input 検査まで辿れるようにする。
final readiness は ready operator execution runbook も必須条件として扱う。`dist/acceptance/aws_dev_uat_operator_execution_runbook.json` がない場合は `missing_operator_runbook`、runbook が `requires_resolved_operator_input` のまま、placeholder を含む、確認なし外部 phase を含む、phase order が崩れている場合は `invalid_operator_runbook` を blocker とする。missing operator runbook では `npm run aws:dev-uat:operator-runbook:build`、`npm run aws:dev-uat:operator-runbook:check`、`npm run aws:dev-uat:operator-runbook:check -- --input dist/acceptance/aws_dev_uat_operator_input.json --require-resolved` を next command に出し、初回 runbook 生成から resolved input を使った ready runbook 検査まで辿れるようにする。
final readiness は current git commit も必須条件として扱う。operator input の `git_commit_sha` / `release.commit_sha`、または operator execution runbook の `git_commit_sha` が現在の検収対象 commit と一致しない場合は `stale_operator_input` / `stale_operator_runbook` を blocker とし、各 artifact を現在 commit で作り直す command を next command に出す。
final readiness は evidence bundle manifest の `schema_version` が `saphnexa-aws-dev-uat-evidence-bundle.v1`、`status` が `checked`、`evidence_class` が `aws-captured`、`git_commit_sha` が current git commit であることを必須条件として扱う。preflight / validation raw input、preflight / validation final evidence、execution bridge の artifact coverage が不足する場合、bundle artifact path が current readiness input path と一致しない場合、bundle artifact の `sha256` / `size_bytes` が現在ファイルと一致しない場合、raw-output を含む全 bundle artifact metadata が現在ファイルと一致しない場合、raw input から導けない範囲外 bundle artifact が混入している場合、または `artifact_count` が artifact 配列と一致しない場合は `invalid_evidence_bundle_manifest`、bundle manifest の git commit が古い場合は `stale_evidence_bundle_manifest` を blocker とし、evidence bundle check command を next command に出す。
`npm run aws:dev-uat:final-readiness:fixture:check` は missing evidence path、invalid evidence bundle manifest、stale evidence bundle manifest、mismatched evidence bundle artifact path、mismatched evidence bundle artifact digest、mismatched raw-output artifact digest、out-of-scope bundle artifact、ready evidence path を fixture で検査する。sample evidence は最終検収 evidence として扱わない。
`npm run aws:dev-uat:operator-handoff:check` は operator handoff artifact を生成し、external action plan、raw capture plan、final readiness manifest、承認必須 action、critical command order、evidence outputs、blockers、next commands、final readiness bundle gate summary を集約する。handoff の evidence input map は preflight / validation raw input path、raw output/input check command、final evidence path、build/final command、evidence bundle manifest path と bundle check command を `required_inputs.evidence` に構造化して記録する。bundle gate summary は artifact count、coverage、metadata match、scope match、invalid/stale flags を `final_readiness_summary.evidence_bundle` に記録する。この command は release 作成、deploy、migration、publish、E2E、負荷試験、Bedrock Evaluations、signoff を実行しない。
`npm run aws:dev-uat:operator-handoff:fixture:check` は operator handoff の pending / requires_confirmation / AWS not-ready branch、evidence input map、final readiness bundle gate summary を fixture で検査する。sample handoff は最終検収 evidence として扱わない。
