...
final readiness は raw input も必須条件として扱う。`dist/acceptance/raw/aws_dev_uat_preflight.raw.json` または `dist/acceptance/raw/aws_dev_uat_validation.raw.json` がない場合は `missing_preflight_raw_input` / `missing_validation_raw_input` を blocker とし、`npm run aws:dev-uat:raw-capture-plan:check`、`npm run aws:dev-uat:raw-input-scaffold:check`、materialize command、raw output check、raw input check を next command に出す。これにより missing raw input から capture plan / scaffold / materialize / raw output-input check まで辿れる。
