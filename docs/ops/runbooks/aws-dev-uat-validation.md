...
`npm run aws:dev-uat:final-readiness:check` は final readiness manifest を生成し、raw capture plan、execution bridge、preflight/validation raw input、final evidence、evidence bundle manifest、blockers、next commands を記録する。実 evidence と AWS credentials が揃わない場合は `blocked_by_external_execution` として残し、`--require-ready` では fail する。evidence bundle manifest は存在だけでなく、`schema_version`、`status`、`evidence_class`、current git commit、artifact count、artifact coverage、current readiness artifact path match を検査する。
...
final readiness は evidence bundle manifest の `schema_version` が `saphnexa-aws-dev-uat-evidence-bundle.v1`、`status` が `checked`、`evidence_class` が `aws-captured`、`git_commit_sha` が current git commit であることを必須条件として扱う。preflight / validation raw input、preflight / validation final evidence、execution bridge の artifact coverage が不足する場合、bundle artifact path が current readiness input path と一致しない場合、または `artifact_count` が artifact 配列と一致しない場合は `invalid_evidence_bundle_manifest`、bundle manifest の git commit が古い場合は `stale_evidence_bundle_manifest` を blocker とし、evidence bundle check command を next command に出す。
`npm run aws:dev-uat:final-readiness:fixture:check` は missing evidence path、invalid evidence bundle manifest、stale evidence bundle manifest、mismatched evidence bundle artifact path、ready evidence path を fixture で検査する。sample evidence は最終検収 evidence として扱わない。
