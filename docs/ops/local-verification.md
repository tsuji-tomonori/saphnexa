...
- Chat UI が `listMessages` route helper / generated operation helper を使い、参加中チャットのユーザー質問と assistant 回答、閲覧者本人の feedback state を source/local gate で再表示できること。他参加者の feedback rating/comment を message history に漏らさないこと。paging cursor、引用本文の完全 REST 復元、実ブラウザ E2E は別途確認する。
...
- Chat message history の local gate は参加者による対象チャットメッセージ一覧取得、ユーザー質問と assistant 回答の復元、閲覧者本人の feedback state 復元、他参加者 feedback 非開示、未参加者拒否を確認する。paging cursor、引用本文の完全 REST 復元、実ブラウザ操作、実 Aurora DSQL での SQL 実行は別途確認する。
