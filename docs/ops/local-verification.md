...
- `npm run aws:dev-uat:final-readiness:fixture:check` が final readiness manifest の missing evidence / missing operator input / invalid operator input / stale operator input / missing operator runbook / invalid operator runbook / stale operator runbook path と ready evidence path を fixture で検査すること。
