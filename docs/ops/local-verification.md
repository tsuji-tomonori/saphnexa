...
- API client route helper と generated operation type map は source gate と実 `tsc --noEmit` で全 public route の operation/path/request/response と主要 outer field 同期を確認する。配列要素や nested object の完全 field-level 型生成、実 CloudFront/Cognito 経由 HTTP request は別途確認する。
