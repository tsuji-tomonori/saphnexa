...
- Admin 評価実行確認の source/local gate は `listEvaluationDatasets`、`startEvaluationRun`、`getEvaluationRun` の route helper / generated operation helper、管理者ロール境界、DSQL plan、評価データセット一覧、評価run詳細と metrics 表示を確認する。実 Step Functions 評価runner、Bedrock Evaluations job、evaluation run items 一覧、評価 HTML report、AppSync fan-out、実ブラウザ E2E は別途実装・検証する。
