...
- `@saphnexa/api-client` が API contract / OpenAPI document 由来の generated operation type map を持ち、method、viewer path、internal path、params、query、request body、success response、error response の operation 単位型を drift check で同期確認すること。
...
- API client route helper と generated operation type map は source gate と実 `tsc --noEmit` で全 public route の operation/path/request/response skeleton 同期を確認する。OpenAPI schema からの詳細 field-level 型生成、実 CloudFront/Cognito 経由 HTTP request は別途確認する。
