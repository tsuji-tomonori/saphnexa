...
- [ ] `issueArtifactAccessCookie` の DSQL query plan が追加される。
- [ ] query plan が active admin actor に限定される。
- [ ] `audit_events` に `admin.artifact.cookie_issued` を append する。
- [ ] response shape が `{ cookie_issued: true, expires_in_seconds: 300 }` を維持する。
- [ ] `packages/api-contract/src/implementation-coverage.ts` 上で `issueArtifactAccessCookie` に planned marker が残らない。
...
- [ ] `npm run api:implementation:check` が成功し、planned marker 数が 9 件になる。
- [ ] `npm run api:implementation:check:production` の失敗リストから `issueArtifactAccessCookie` が消える。
