...
- [ ] validation raw capture plan が materializer command を明示する。
- [ ] materializer command が validation raw output capture 後、raw output/input check と validation build 前の導線として検査される。
- [ ] validation scaffold の operator notes が materializer 実行と scaffold 非 final evidence を明示する。
- [ ] raw capture plan / scaffold / docs / CI check が materializer command を要求する。
- [ ] 実 AWS dev/UAT 検証を完了扱いにしない。
