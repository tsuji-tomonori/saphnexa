...
- [ ] 対象 API 3件の DSQL query plan が `chat_participant_events` に domain event を append する。
- [ ] 対象 API 3件の DSQL query plan が `audit_events` に audit event を append する。
...
- [ ] `npm run api:implementation:check` が成功し、planned marker 数が 18 件になる。
